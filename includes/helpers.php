...
function conseguirUsuario($conexion){
...
function conseguirBusquedaColonia($conexion,$busqueda){
	if(!empty($busqueda)){
...
function conseguirBusquedaTipo($conexion,$busqueda){
	if(!empty($busqueda)){
...
function conseguirBusquedaCpostal($conexion,$busqueda){
	if(!empty($busqueda)){
	$sql = "SELECT * FROM codigopostal ".
			"WHERE codigopostal LIKE '%$busqueda%' ";
...
function conseguirComentarios($conexion){
	$sql = "SELECT c.*, u.nombre AS 'usuario', u.apellido AS 'apellido' FROM comentario c ".
			"INNER JOIN usuarios u ON c.usuarios_id = u.id ".
			"WHERE c.responder = 0 ".
			"ORDER BY c.fecha DESC";

	$comentariosYa = mysqli_query($conexion, $sql);
	
	$resultado = array();
	if($comentariosYa && mysqli_num_rows($comentariosYa) >= 1){
		$resultado = $comentariosYa;
	}		
	return $comentariosYa;
}

function responderComentarios($conexion, $idRespond){
	$sql = "SELECT c.*, u.nombre AS 'usuario', u.apellido AS 'apellido' FROM comentario c ".
			"INNER JOIN usuarios u ON c.usuarios_id = u.id ".
			"WHERE c.responder = $idRespond ";

	$responderYa = mysqli_query($conexion, $sql);
	
	$resultado = array();
	if($responderYa && mysqli_num_rows($responderYa) >= 1){
		$resultado = $responderYa;
	}		
	return $responderYa;
}

function usuarioComentarios($conexion, $user){
	$sql = "SELECT * FROM usuarios WHERE nombre = '$user' ";

	$usercoment = mysqli_query($conexion, $sql);
	
	$resultado = array();
	if($usercoment && mysqli_num_rows($usercoment) >= 1){
		$resultado = $usercoment;
	}		
	return $resultado;
}

function conseguirMensajes($conexion, $user){
	$sql = "SELECT m.*, u.nombre AS 'usuario', u.apellido AS 'apellido' FROM mensajes m ".
			"INNER JOIN usuarios u ON m.de = u.id ".
			"WHERE m.para = $user ".
			"ORDER BY m.fecha DESC";
	$mensaje = mysqli_query($conexion, $sql);

	$result = array();
	if($mensaje && mysqli_num_rows($mensaje) >= 1){
		$result = $mensaje;
	}
	return $result;
}

function mostrarMensaje($conexion,$mensaje){
